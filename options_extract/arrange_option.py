...
def contains_cmake_function(code):
    cmake_functions = [
        "set(", "unset(", "list(",  "string(", "add_definitions(",
        "set_property(","find_package(","option(","find_library(","pkg_check_modules("
    ]
    ...
    with open(input_file, newline='', encoding='utf-8') as infile:
        ...
        for row in reader:
            ...
            if dependency_name not in reachability_data:
                reachability_data[dependency_name] = {'options': [], 'actor_id': actor_id}  # 保存 actor_id
            
            reachability_data[dependency_name]['options'].extend(options)
...
def search_name_column(input_file, reachability_data):
    ...
    with open(input_file, newline='', encoding='utf-8') as infile:
        ...
        for row in reader:
            ...
            for dependency_name_key, data in reachability_data.items():
                for option in data['options']:
                    if is_exact_match(option, name):
                        ...
                        if not any("package_actor_id" in entry for entry in result_data[dependency_name_key]):
                            result_data[dependency_name_key].append({
                                'package_actor_id': data['actor_id'],  
                            })
