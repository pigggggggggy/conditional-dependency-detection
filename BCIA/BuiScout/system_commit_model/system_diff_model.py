...
class SystemDiff(object):
    ...
    def set_file_data_non_modified_only(self):
        ...
        if not self.file_data:
            self.file_data = {
                "no_cmake_files_found": {
                    "commit_hash": self.commit.hash,
                    "file_name": "No CMake files found",
    ...
    def perform_data_flow_analysis(self):
        if self.cdus_extracted:
            return
        if "no_cmake_files_modified" in self.file_data or "no_cmake_files_found" in self.file_data:
            print(f"Commit {self.commit.hash} does not involve CMake files, but continuing analysis...")
    ...
    def analyze_global(self):
        if "no_cmake_files_modified" in self.file_data or "no_cmake_files_found" in self.file_data:
            print(f"Analyzing commit {self.commit.hash} without CMake file changes...")
